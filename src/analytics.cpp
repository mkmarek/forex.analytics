...
void getTrades(const FunctionCallbackInfo<Value>& args){
    ...
    std::vector<std::map<std::string, IndicatorData> > dataSet = system
...
void getIndicatorData(const FunctionCallbackInfo<Value>& args){
    ...
    std::vector<std::map<std::string, IndicatorData>> indicatorData =
    ...
    for (unsigned long i = 0; i < indicatorData.size(); i++) {
      ...
      for(it_type iterator = indicatorData[i].begin(); iterator != indicatorData[i].end(); iterator++) {
