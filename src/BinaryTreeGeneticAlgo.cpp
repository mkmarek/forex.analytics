...
void BinaryTreeGeneticAlgo::Mutate(TreeNode * node){
    if (node->left != nullptr && node->right != nullptr) {
        ...
        double logicalNodeMutation = ((double)(rand() % 100)) / 100;
        ...
        IndicatorTreeNode * inNode = (IndicatorTreeNode *)node;

        double leafValueMutation = ((double)(rand() % 100)) / 100;
        double leafValueSignMutation = ((double)(rand() % 100)) / 100;
        double leafValueIndicatorMutation = ((double)(rand() % 100)) / 100;
...
void BinaryTreeGeneticAlgo::Crossover(std::vector<BinaryTreeChromosome *>& generation){
    for (unsigned long i = this->select + 1; i < generation.size(); i++) {
        double crossover = ((double)(rand() % 100)) / 100;
